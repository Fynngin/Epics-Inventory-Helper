let userId = JSON.parse(JSON.parse(window.localStorage.getItem("persist:root"))["auth"])[
	"user"
]["id"];
...
chrome.runtime.onMessage.addListener(function (request) {
	if (request.message === "tradeView") {
		...
		window.setTimeout(async function () {
			...
			let leftHeader =
				leftSide.parentElement.previousSibling.childNodes[0].querySelector("p");
			let rightHeader =
				leftSide.parentElement.previousSibling.childNodes[1].querySelector("p");
			...
			getTradeInfo(request.tradeId, request.categoryId, function (tradeInfo) {
				sortCards(leftCards, rightCards, tradeInfo, function (items) {
					injectMarketValues(items, request.categoryId, jwt, function (cardsUpdated) {
						let leftSum = cardsUpdated.leftSide
							.map((it) => it.price)
							.reduce((leftSum, current) => leftSum + current, 0);
						let rightSum = cardsUpdated.rightSide
							.map((it) => it.price)
							.reduce((rightSum, current) => rightSum + current, 0);
...
function sortCards(leftCards, rightCards, tradeInfo, callback) {
	...
	for (let item of leftCards) {
		res.leftSide.push(
			findMatch(item, incoming ? tradeInfo["user2"] : tradeInfo["user1"])
		);
	}
	for (let item of rightCards) {
		res.rightSide.push(
			findMatch(item, incoming ? tradeInfo["user1"] : tradeInfo["user2"])
		);
	}
	callback(res);
}
...
function findMatch(item, tradeInfo) {
	let src = item.querySelector("img").src.split("?")[0].split(".epics.gg")[1];
	let template;
	if (src.match("card")) {
		template = tradeInfo["cards"].find(
			(el) =>
				el["cardTemplate"]["images"]["size402"]
					.split("?")[0]
					.split(".epics.gg")[1]
					.split("/")[5] === src.split("/")[5] ||
				el["images"]["size402"].split("?")[0].split(".epics.gg")[1].split("/")[5] ===
					src.split("/")[5]
		);
		return {
			type: "card",
		...
		template = tradeInfo["stickers"].find(
			(el) => el["stickerTemplate"]["images"][1].url === src
		);
		return {
			type: "sticker",
			item: item,
